...
func TestUpdateNotificationInterstitial_SkipsSticky(t *testing.T) {
	s := newTestServer(t)
	ctx := context.Background()
	...
	if updated.EveryN == nil || *updated.EveryN != 1 {
		t.Fatalf("expected everyN to remain 1 for sticky, got %v", updated.EveryN)
	}
}
